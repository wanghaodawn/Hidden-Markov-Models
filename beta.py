# 
# This file finished the Backward Algorithm of Hidden Markov Models
# By Hao Wang - haow2
# 04/08/2016
# 
	...
	while 1:
		...
		while i >= 0:
			...
			for k in range(0, 8):
				...
				for j in range(0, 8):
					# print("after_list: " + str(len(after_list)))
					# print("list_trans: " + str(len(list_trans[k])))
					# print("j: " + str(j))
					# print("word: " + word)
					temp_temp = after_list[j] + list_trans[k][j] + list_emit[j][words[i+1]]
					if j == 0:
						temp = temp_temp
					else:
		...
		res = 0.0
		for j in range(0, 8):
			temp = curr_list[j] + list_prior[j] + list_emit[j][words[0]]
			if j == 0:
				res = temp
			else:
				res = log_sum(res, temp)
		print(res)
